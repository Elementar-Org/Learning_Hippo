...
*Rappresentazione del modello a cinque compartimenti per un neurone piramidale di Hippo-1 con le rispettive equazioni di integrazione del segnale. Ogni equazione incorpora il meccanismo di shunting inhibition, dove un segnale eccitatorio $X\_i$ è modulato moltiplicativamente da un segnale inibitorio $Y\_i$.*

Le formule che governano l'integrazione nei vari compartimenti sono:

$$
\begin{align}
R_{dist_A} &= \zeta(X_{1})\bigl(1 - s_{1}\,\zeta(Y_{1})\bigr) \\
R_{dist_B} &= \zeta(X_{2})\bigl(1 - s_{2}\,\zeta(Y_{2})\bigr) \\
R_{prox} &= \bigl(R_{dist_A}+R_{dist_B}+\zeta(X_{3})\bigr)\bigl(1 - s_{3}\,\zeta(Y_{3})\bigr) \\
R_{bas}  &= \zeta(X_{4})\bigl(1 - s_{4}\,\zeta(Y_{4})\bigr) \\
R_{soma} &= \bigl(R_{prox}+R_{bas}+\zeta(X_{5})\bigr)\bigl(1 - s_{5}\,\zeta(Y_{5})\bigr)
\end{align}
$$dove $s\_i$ è un coefficiente di *shunting* addestrato che determina l'intensità dell'inibizione.
...
```bibtex
@article{corradetti2025learning,
title={Learning Hippo: A Hippocampal Neural Architecture for Deep Learning},
author={Corradetti, Daniele and Gallo, Matteo and Gnesi, Ivan and Marrani, Alessio and Costanti, Filippo and Corradetti, Renato},
